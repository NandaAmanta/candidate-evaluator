...
async def login_for_access_token(data : Login, service: AuthService = Depends()):
    ...
    return BaseApiResponse[UserReadWithToken](data=user, message="Success", status_code=200).make()


@router.post("/registration", status_code=201)
async def register_new_user(data : UserCreation, service: AuthService = Depends()):
    user = service.register(data) 
    return BaseApiResponse[UserRead](data=user, message="User registered successfully", status_code=201).make()
